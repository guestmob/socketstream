...
// Cross-browser console.log() via window.log()
...
else{setTimeout(function(){log(Array.prototype.slice.call(arguments));},500);}}}};((function(){var RTM,backendError,connectionString,default_cb,getCookie,output,sendHeartbeat,setCookie,setupAPI,start,validLevel,__slice=Array.prototype.slice;window.SS={started:null,env:null,client:{},server:{},shared:{},models:{},internal:{cb_stack:{}},config:{log:{level:0}}},SS.events={_events:{},on:function(name,fn){return this._events[name]==null&&(this._events[name]=[]),this._events[name].push(fn)},emit:function(){var event,name,params,_i,_len,_ref,_results;name=arguments[0],params=2<=arguments.length?__slice.call(arguments,1):[];if(this._events[name]){_ref=this._events[name],_results=[];for(_i=0,_len=_ref.length;_i<_len;_i++)event=_ref[_i],_results.push(event.apply(event,params));return _results}return console.error("Error: Received incoming '"+name+"' event but no event handlers registered")}},connectionString=function(){var port;return port=document.location.port.length===0?document.location.protocol==="https:"?443:80:document.location.port,document.location.protocol+"//"+document.location.hostname+":"+port},SS.socket=io.connect(connectionString()),default_cb=function(server_response){return console.log(server_response)},SS.internal.remote=function(){var args,cb,last_arg,msg;return args=Array.prototype.slice.call(arguments),last_arg=args[args.length-1],typeof last_arg!="function"&&args.push(default_cb),msg={},msg.method=args[0],SS.config.remote_prefix&&(msg.method=""+SS.config.remote_prefix+"."+msg.method),msg.params=args.length>1?args.slice(1,args.length-1):null,validLevel(4)&&(!msg.options||!msg.options.silent)&&log("<- "+msg.method),cb=args.pop(),SS.socket.emit("server",msg,function(data){return data.error?backendError(data.error):(output(2,"-> "+msg.method,data.result),cb(data.result))})},RTM=function(){function RTM(){}return RTM.prototype.findById=function(id,cb){return this._send("findById",id,cb)},RTM.prototype.find=function(){var arg,args,cb,_i,_len;args=[];for(_i=0,_len=arguments.length;_i<_len;_i++)arg=arguments[_i],args.push(arg);return cb=args.pop(),this._send("find",args,cb)},RTM.prototype.count=function(){var arg,args,cb,_i,_len;args=[];for(_i=0,_len=arguments.length;_i<_len;_i++)arg=arguments[_i],args.push(arg);return cb=args.pop(),this._send("count",args,cb)},RTM.prototype._send=function(action,params,cb){return log(2,"<~ "+this.name+"."+action),SS.socket.emit("rtm",{rtm:this.name,action:action,params:params},cb)},RTM}(),SS.socket.on("getSessionID",function(data,cb){return cb(getCookie("session_id"))}),SS.socket.on("init",function(msg){var data,name,_i,_len,_ref;data=JSON.parse(msg),SS.env=data.env,SS.config=data.config||{},data.session_id&&setCookie("session_id",data.session_id),_ref=data.api.models;for(_i=0,_len=_ref.length;_i<_len;_i++)name=_ref[_i],SS.models[name]=new RTM,SS.models[name].name=name;return eval("SS.server = "+data.api.server),setupAPI(SS.server,[]),SS.socket.ready=!0,data.heartbeat&&sendHeartbeat(),start()}),SS.socket.on("reload",function(){if(SS.config.auto_reload)return log("Reloading as files have changed..."),window.location.reload()}),SS.socket.on("event",function(name,params,destination){var info;return info=destination&&" ["+destination+"]"||"",output(2,"=> "+name+info),SS.events.emit(name,params,destination)}),SS.socket.on("rtm",function(msg){var cb,data;data=JSON.parse(msg),cb=SS.internal.cb_stack[data.id];if(cb)return output(2,"~> "+cb.msg.rtm+"."+cb.msg.action),cb.funkt(data.data),delete SS.internal.cb_stack[data.id]}),start=function(){var init;if(!SS.started)return jQuery?jQuery(document).ready(function(){var init;return console.log("hello from jquery"),init=SS.client.app.init,init()}):(console.log("hello from wout jquery"),init=SS.client.app.init,init()),SS.started=new Date},backendError=function(error){var msg;return msg="SocketStream Server Error: ",error.code&&(msg+=error.code+" - "),msg+=error.message,error.stack&&(msg+="\n"+error.stack),console.error(msg)},output=function(level,msg,params){var o;if(validLevel(level))return o=[msg],params&&validLevel(3)&&o.push(params),log.apply(log,o)},validLevel=function(level){return SS.config.log.level>=level},setupAPI=function(root,ary){var key,ns,value,_results;_results=[];for(key in root)value=root[key],ns=ary.slice(0),ns.push(key),_results.push(typeof value=="object"?setupAPI(root[key],ns):root[key]=new Function('SS.internal.remote.apply(window, ["'+ns.join(".")+'"].concat(Array.prototype.slice.call(arguments, 0)))'));return _results},sendHeartbeat=function(){return SS.socket.socket.connected&&SS.socket.emit("heartbeat"),setTimeout(arguments.callee,SS.config.heartbeat_interval*1e3)},getCookie=function(c_name){var c_end,c_start;if(document.cookie.length>0){c_start=document.cookie.indexOf(c_name+"=");if(c_start!==-1)return c_start=c_start+c_name.length+1,c_end=document.cookie.indexOf(";",c_start),c_end===-1&&(c_end=document.cookie.length),unescape(document.cookie.substring(c_start,c_end))}return""},setCookie=function(c_name,value,expiredays){var exdate;return exdate=new Date,exdate.setDate(exdate.getDate()+expiredays),document.cookie=""+c_name+"="+escape(value)+(expiredays===null?"":";expires="+exdate.toUTCString())}})).call(this)